...
spt_e _CheckType<false>(const _pk_t* pkarr, _size_t n)
{
    ...
    for (_index_t i = 0; i < n; ++i)
        if (_pkMaxValue < (_vtsdb += _sdb_pk_value(pkarr[i])))
            return spt_spillout;    // 如果值过大直接爆点

    if (_pkMaxCount == n)   // 如果张数最大
    {
        if (_vtsdb < _pkMaxValue)   // 没有满点
        {
...
spt_e _CheckType<true>(const _pk_t* pkarr, _size_t n)
{
    ...
    for (_index_t i = 0; i < n; ++i)
        if (_pkFlower(pkarr[i]) != _fK)     // 统计非王牌的点数
            if (_pkMaxValue - 1 < (_vtsdb += _sdb_pk_value(pkarr[i])))
                return spt_spillout;        // 如果值过大直接爆点

    if (1 == nCount)    // 如果只有一个王
    {
        if (_vtsdb == (_pkMaxValue - 1) || 1 == (_vtsdb % 2))
            _vtsdb = _pkMaxValue;
        else
            _vtsdb = _pkMaxValue - 1;
